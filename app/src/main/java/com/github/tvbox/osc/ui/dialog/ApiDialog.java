...
public class ApiDialog extends BaseDialog {
    ...
    private final EditText inputEPGLogo;
    ...
    public void refresh(RefreshEvent event) {
        ...
        if (event.type == RefreshEvent.TYPE_EPG_LOGO_URL_CHANGE) {
            inputEPGLogo.setText((String) event.obj);
        }
    ...
    public ApiDialog(@NonNull @NotNull Context context) {
        ...
        inputEPGLogo = findViewById(R.id.input_epg_logo);
        inputEPGLogo.setText(Hawk.get(HawkConfig.EPG_LOGO_URL, ""));
        ...
        findViewById(R.id.inputSubmit).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String newEPGLogo = inputEPGLogo.getText().toString().trim();
                ...
                // Capture EPG LOGO input into Settings
                Hawk.put(HawkConfig.EPG_LOGO_URL, newEPGLogo);
                if (!newEPGLogo.isEmpty()) {
                    ArrayList<String> EPGLogoHistory = Hawk.get(HawkConfig.EPG_LOGO_HISTORY, new ArrayList<String>());
                    if (!EPGLogoHistory.contains(newEPGLogo))
                        EPGLogoHistory.add(0, newEPGLogo);
                    if (EPGLogoHistory.size() > 20)
                        EPGLogoHistory.remove(20);
                    Hawk.put(HawkConfig.EPG_LOGO_HISTORY, EPGLogoHistory);
                }
        ...
        findViewById(R.id.EPGLogoHistory).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ArrayList<String> EPGLogoHistory = Hawk.get(HawkConfig.EPG_LOGO_HISTORY, new ArrayList<String>());
                if (EPGLogoHistory.isEmpty())
                    return;
                String current = Hawk.get(HawkConfig.EPG_LOGO_URL, "");
                int idx = 0;
                if (EPGLogoHistory.contains(current))
                    idx = EPGLogoHistory.indexOf(current);
                ApiHistoryDialog dialog = new ApiHistoryDialog(getContext());
                dialog.setTip(HomeActivity.getRes().getString(R.string.dia_history_epg_logo));
                dialog.setAdapter(new ApiHistoryDialogAdapter.SelectDialogInterface() {
                    @Override
                    public void click(String epgLogoURL) {
                        inputEPG.setText(epgLogoURL);
                        Hawk.put(HawkConfig.EPG_LOGO_URL, epgLogoURL);
                        dialog.dismiss();
                    }

                    @Override
                    public void del(String value, ArrayList<String> data) {
                        Hawk.put(HawkConfig.EPG_LOGO_HISTORY, data);
                    }
                }, EPGLogoHistory, idx);
                dialog.show();
            }
